    ...
    def tf(self, img, k=0): 
        ...
        for bs in range(bsize): 
            if k == 0: # image 
                ...
                channels = [rotate(img[bs,:,:,:,c], self.angle_buffer, axes=self.axes_buffer, reshape=False, order=1, mode='constant', cval=0) for c in range(img.shape[4])] 
...
class RandomIntensityChange(Base):
    ...
    def tf(self,img,k=0):
        ...
        B, H, W, D, C = img.shape
        shift_factor = np.random.uniform(-self.shift, self.shift, size=(B,1,1,1,C)).astype(img.dtype)
        scale_factor = np.random.uniform(1.0-self.scale, 1.0+self.scale, size=(B,1,1,1,C)).astype(img.dtype)
...
class GaussianBlur(Base):
    def __init__(self, dim, sigma=Constant(1.5), num=1):
        # 1.5 pixel
        self.dim = dim
        self.sigma = sigma
        self.eps   = 0.001
        self.num = num # apply only to the first num items (image=0 -> num=1)
